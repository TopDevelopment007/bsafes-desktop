...
function getPageItem(thisItemId, thisExpandedKey, thisPrivateKey, thisSearchKey, done, thisVersion) {
    ...
    $.post(server_addr + '/memberAPI/getPageItem', options, function(data, textStatus, jQxhr) {
        if (data.status === 'ok') {
            ...
            console.log('** (err_getPageItem_response)', data.error, thisItemId);
            done(data.error, null)
        }
        
    }, 'json')
    .fail(function(jqXHR, textStatus, errorThrown){
        console.log('** (err_getPageItem_ajax)');
        processErrors(jqXHR);
    });
}
...
function downloadNextContentImageObject(item_content, itemId) {
    ...
    for (var i = 0; i < encryptedImages.length; i++) {
        downloadImageObject($(encryptedImages[i]), itemId, i);
    }

}
;
function downloadImageObject(encryptedImageElement, itemId, index) {
...
function downloadVideoObject($videoDownload, index) {
...
function handleVideoObjects(item_content, itemId) {
    ...
    for (var i = 0; i < videoDownloads.length; i++) {
        downloadVideoObject($(videoDownloads[i]), i);
    }
}
...
var downloadAttachment = function(id, index) {
        ...
        function downloadAChunk(signedURL) {
            ...
            xhr.addEventListener("progress", function(evt) {
                ...
                if (evt.lengthComputable) {
                    ...
                    console.log('== (attachment)', index + 1, id, attachmentFileProgress);
                    saveLog('Attachment ' + (index + 1).toString() + ' downloading : ' + attachmentFileProgress + '%', id);
                }
            }, false);
